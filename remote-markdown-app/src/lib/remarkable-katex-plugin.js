 ...
 * Katex package.
 */
export default function rkatex(md, options) {
  const backslash = '\\';
  const dollar = '$';
  const opts = options || {};
   ...
   */
  const renderKatex = (source, displayMode) => katex.renderToString(source, {displayMode: displayMode, throwOnError: false});
  ...
  const parseBlockKatex = (state, startLine, endLine) => {
    ...
    if (marker !== delimiter.charCodeAt(0)) { return false; }
    ...
    state.tokens.push({type: 'katex', params: null, content: content, lines: [startLine, state.line], level: state.level, block: true});
    return true;
  };
  ...
  // block parser のルールの中に katex を paragraph より前に入れると $$ を
  // parseBlockKatex が担当することになる
  // そうしなくても paragraph によって <p>$$\nx + y\n$$</p> のように解釈されてから
  // parseInlineKatex で処理されるので同じことだが，せっかく parseBlockKatex を
  // 作ったので使いたい
  md.block.ruler.before('paragraph', 'katex', parseBlockKatex, options);
